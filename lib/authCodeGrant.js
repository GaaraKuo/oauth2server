  ...
  this.model.getClient(this.clientId, null, function (err, client) {
    ...
    if (!client) {
      ...
      var uri = url.parse(client.redirectUri, true)
			...
			uri.search = null;
      uri.pathname = null;
      console.log("client:" + client.redirectUri)
      console.log("self:" + self.redirectUri)
      if (url.format(uri) !== self.redirectUri) {
        return done(error('invalid_request', 'redirect_uri does not match'));
      }
    }
...
function redirect (done) {
  var uri = url.parse(this.client.redirectUri, true);
